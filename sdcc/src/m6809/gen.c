...
static struct asmop asmop_a, asmop_b, asmop_c, asmop_d, asmop_e, asmop_h, asmop_l, asmop_iyh, asmop_iyl, asmop_hl, asmop_zero,
...
static void
cost2 (unsigned int bytes, unsigned int cycles_z80, unsigned int cycles_z180, unsigned int cycles_r2k,
       unsigned int cycles_gbz80, unsigned int cycles_tlcs90, unsigned int cycles_ez80_z80)
{
  regalloc_dry_run_cost += bytes;
}
...
sameRegs (asmop * aop1, asmop * aop2)
{
  ...
  if (!regalloc_dry_run &&      // Todo: Check if always enabling this even for dry runs tends to result in better code.
      (aop1->type == AOP_STK && aop2->type == AOP_STK || aop1->type == AOP_EXSTK && aop2->type == AOP_EXSTK))
    {
...
fetchLitPair (PAIR_ID pairId, asmop * left, int offset)
{
  ...
  if (IS_RAB && pairId == PAIR_HL && left->type == AOP_LIT && !byteOfVal (left->aopu.aop_lit, offset + 1)
      && !byteOfVal (left->aopu.aop_lit, offset))
    {
      emit2 ("bool hl");
      emit2 ("ld l, h");
      regalloc_dry_run_cost += 2;
...
fetchPairLong (PAIR_ID pairId, asmop * aop, const iCode * ic, int offset)
{
  ...
  if (isLitWord (aop))
  ...
  else
    {
      ...
      /* we need to get it byte by byte */
      else if (pairId == PAIR_HL && (IS_GB || (IY_RESERVED && (aop->type == AOP_HL || aop->type == AOP_EXSTK)))
               && requiresHL (aop))
        {
      ...
      else
        {
          ...
          /* The Rabbit's cast to bool is a cheap way of zeroing h (similar to xor a, a for a for the Z80). */
                   ...
                   && !(aop->type == AOP_REG
                        && (aop->aopu.aop_reg[offset]->rIdx == L_IDX || aop->aopu.aop_reg[offset]->rIdx == H_IDX)))
            {
              emit2 ("bool hl");
              regalloc_dry_run_cost++;
              if (!regalloc_dry_run)
...
/*-----------------------------------------------------------------*/
static void
pointPairToAop (PAIR_ID pairId, const asmop * aop, int offset)
{
  switch (aop->type)
    {
    case AOP_EXSTK:
    ...
    case AOP_STK:
      ...
      if ((_G.pairs[pairId].last_type == AOP_STK || _G.pairs[pairId].last_type == AOP_EXSTK)
      ...
      switch (aop->type)
        ...
        case AOP_STK:
          if (IS_TLCS90)        // Try to use (sp) addressing mode.
            {
              int sp_offset =
                aop->aopu.aop_stk + offset + (aop->aopu.aop_stk >
                                              0 ? _G.stack.param_offset : 0) + _G.stack.pushed + _G.stack.offset;
...
cheapMove (asmop * to, int to_offset, asmop * from, int from_offset, bool a_dead)
{
  ...
  if ((to->type == AOP_STK || to->type == AOP_EXSTK) && _G.omitFramePtr &&
      (aopInReg (to, to_offset, A_IDX) || aopInReg (to, to_offset, B_IDX) || aopInReg (to, to_offset, D_IDX)
       || aopInReg (to, to_offset, H_IDX) || aopInReg (to, to_offset, IYH_IDX)))
    {
      ...
      if (!sp_offset)
        {
          emit2 ("inc sp");
          emit2 ("push %s",
                 aopInReg (from, from_offset,
                           A_IDX) ? "af" : (aopInReg (from, from_offset,
                                            B_IDX) ? "bc" : (aopInReg (from, from_offset,
                                                D_IDX) ? "de" : (aopInReg (from, from_offset,
                                                    H_IDX) ? "hl" :
                                                    "iy"))));
          emit2 ("inc sp");
      ...
      else
        {
          switch (id)
            ...
            case PAIR_BC:
              ...
              break;
            case PAIR_DE:
              if (!IS_GB && aop->type == AOP_REG && aop->aopu.aop_reg[0]->rIdx == L_IDX && aop->aopu.aop_reg[1]->rIdx == H_IDX
                  && !dont_destroy)
                {
                  emit2 ("ex de, hl");
                  regalloc_dry_run_cost++;
            ...
            case PAIR_HL:
              ...
              else if (!IS_GB && aop->type == AOP_REG && aop->aopu.aop_reg[0]->rIdx == E_IDX
...
genCopy (asmop * result, int roffset, asmop * source, int soffset, int sizex, bool a_dead, bool hl_dead)
{
  ...
  for (int i = 0; i < n; i++)
    if (result->type == AOP_REG && source->type == AOP_REG
        && result->aopu.aop_reg[roffset + i] == source->aopu.aop_reg[soffset + i])
      {
        assigned[i] = true;
        regsize--;
        size--;
  ...
  for (int i = 0; i < n;)
    {
      if (i + 1 < n && result->type == AOP_STK &&
          (aopInReg (source, soffset + i, HL_IDX) && IS_RAB ||
           (aopInReg (source, soffset + i, BC_IDX) || aopInReg (source, soffset + i, DE_IDX)
            || aopInReg (source, soffset + i, HL_IDX) || aopInReg (source, soffset + i, IY_IDX)) && (IS_EZ80_Z80 || IS_TLCS90)))
        {
          if (!regalloc_dry_run)
            {
  ...
  for (int i = 0; i + 1 < n; i++)
    {
      ...
      for (int j = 0; j + 1 < n; j++)
        {
          if (!assigned[j] && i != j && i + 1 != j && !aopOnStack (result, roffset + i, 2)
              && !aopOnStack (source, soffset + i, 1) && (result->aopu.aop_reg[roffset + i] == source->aopu.aop_reg[soffset + j]
  ...
  for (int i = 0; i < n;)
    {
      if (i + 1 < n && source->type == AOP_STK &&
          (aopInReg (result, roffset + i, HL_IDX) && IS_RAB ||
           (aopInReg (result, roffset + i, BC_IDX) || aopInReg (result, roffset + i, DE_IDX)
            || aopInReg (result, roffset + i, HL_IDX) || aopInReg (result, roffset + i, IY_IDX)) && (IS_EZ80_Z80 || IS_TLCS90)))
        {
          if (!regalloc_dry_run)
            {
  ...
  if (size)
    {
      ...
      hl_free = hl_dead && (result->regs[L_IDX] < 0 || result->regs[L_IDX] >= roffset + source->size)
                && (result->regs[H_IDX] < 0 || result->regs[H_IDX] >= roffset + source->size);
      if (!a_free)
        {
          _push (PAIR_AF);
        }
...
genMove_o (asmop * result, int roffset, asmop * source, int soffset, int size, bool a_dead_global, bool hl_dead_global)
{
  ...
  if ((result->type == AOP_REG || !IS_GB && result->type == AOP_STK)
      && (source->type == AOP_REG || !IS_GB && source->type == AOP_STK))
    {
      ...
      roffset += csize;
      size -= csize;
      genMove_o (result, roffset, ASMOP_ZERO, 0, size, a_dead_global && result->regs[A_IDX] < roffset, hl_dead_global
                 && result->regs[H_IDX] < roffset && result->regs[L_IDX] < roffset);
      return;
    }
  ...
  for (unsigned int i = 0; i < size;)
    {
      ...
      if (result->type == AOP_HL && a_dead_global
          && (!hl_dead_global || source->regs[L_IDX] != -1 || source->regs[H_IDX] != -1))
        {
  ...
  if (_G.saves.saved == FALSE)
    {
      ...
      if (options.oldralloc)
        ...
        {
          push_bc = bitVectBitValue (ic->rSurv, B_IDX) && !ftype->funcAttrs.preserved_regs[B_IDX]
                    || bitVectBitValue (ic->rSurv, C_IDX) && !ftype->funcAttrs.preserved_regs[C_IDX];
          push_de = bitVectBitValue (ic->rSurv, D_IDX) && !ftype->funcAttrs.preserved_regs[D_IDX]
                    || bitVectBitValue (ic->rSurv, E_IDX) && !ftype->funcAttrs.preserved_regs[E_IDX];
  ...
  while (size)
    {
      ...
      bool a_free = !bitVectBitValue (ic->rSurv, A_IDX) && (IC_LEFT (ic)->aop->regs[A_IDX] < 0
                    || IC_LEFT (ic)->aop->regs[A_IDX] >= size - 1);
      bool b_free = !bitVectBitValue (ic->rSurv, B_IDX) && (IC_LEFT (ic)->aop->regs[B_IDX] < 0
                    || IC_LEFT (ic)->aop->regs[B_IDX] >= size - 1);
      bool c_free = !bitVectBitValue (ic->rSurv, C_IDX) && (IC_LEFT (ic)->aop->regs[C_IDX] < 0
                    || IC_LEFT (ic)->aop->regs[C_IDX] >= size - 1);
      bool d_free = !bitVectBitValue (ic->rSurv, D_IDX) && (IC_LEFT (ic)->aop->regs[D_IDX] < 0
                    || IC_LEFT (ic)->aop->regs[D_IDX] >= size - 1);
      bool e_free = !bitVectBitValue (ic->rSurv, E_IDX) && (IC_LEFT (ic)->aop->regs[E_IDX] < 0
                    || IC_LEFT (ic)->aop->regs[E_IDX] >= size - 1);
      bool h_free = !bitVectBitValue (ic->rSurv, H_IDX) && (IC_LEFT (ic)->aop->regs[H_IDX] < 0
                    || IC_LEFT (ic)->aop->regs[H_IDX] >= size - 1);
      bool l_free = !bitVectBitValue (ic->rSurv, L_IDX) && (IC_LEFT (ic)->aop->regs[L_IDX] < 0
                    || IC_LEFT (ic)->aop->regs[L_IDX] >= size - 1);
      bool hl_free = isPairDead (PAIR_HL, ic) && (h_free || IC_LEFT (ic)->aop->regs[H_IDX] >= size - 2) && (l_free
                     || IC_LEFT (ic)->
                     aop->regs[L_IDX] >=
                     size - 2);
      bool de_free = isPairDead (PAIR_DE, ic) && (d_free || IC_LEFT (ic)->aop->regs[D_IDX] >= size - 2) && (e_free
                     || IC_LEFT (ic)->
                     aop->regs[E_IDX] >=
                     size - 2);
      bool bc_free = isPairDead (PAIR_BC, ic) && (b_free || IC_LEFT (ic)->aop->regs[B_IDX] >= size - 2) && (c_free
                     || IC_LEFT (ic)->
                     aop->regs[C_IDX] >=
                     size - 2);
      ...
      if (getPairId_o (IC_LEFT (ic)->aop, size - 2) != PAIR_INVALID)
        {
          ...
          d = 2;
               ...
               (hl_free || de_free || bc_free ||
                aopInReg (IC_LEFT (ic)->aop, size - 1, B_IDX) && c_free || b_free
                && aopInReg (IC_LEFT (ic)->aop, size - 2, C_IDX) || aopInReg (IC_LEFT (ic)->aop, size - 1, D_IDX) && e_free
                || d_free && aopInReg (IC_LEFT (ic)->aop, size - 2, E_IDX) || aopInReg (IC_LEFT (ic)->aop, size - 1, H_IDX)
                && l_free || h_free && aopInReg (IC_LEFT (ic)->aop, size - 2, L_IDX)))
        {
          ...
          if (aopInReg (IC_LEFT (ic)->aop, size - 1, H_IDX) && l_free || h_free
              && aopInReg (IC_LEFT (ic)->aop, size - 2, L_IDX))
            {
              pair = PAIR_HL;
            }
          else if (aopInReg (IC_LEFT (ic)->aop, size - 1, D_IDX) && e_free || d_free
                   && aopInReg (IC_LEFT (ic)->aop, size - 2, E_IDX))
            {
              pair = PAIR_DE;
            }
          else if (aopInReg (IC_LEFT (ic)->aop, size - 1, B_IDX) && c_free || b_free
                   && aopInReg (IC_LEFT (ic)->aop, size - 2, C_IDX))
            {
              pair = PAIR_BC;
            }
      ...
      else if (size >= 2 && IS_Z80N && (IC_LEFT (ic)->aop->type == AOP_LIT || IC_LEFT (ic)->aop->type == AOP_IMMD))     // Same size, but slower (21 vs 23 cycles) than going through a register pair other than iy. Only worth it under high register pressure.
        {
          ...
          d = 2;
               ...
               && (IC_LEFT (ic)->aop->type == AOP_LIT || IC_LEFT (ic)->aop->type == AOP_IMMD))
        {
...
genCall (const iCode * ic)
{
  ...
  if (bigreturn)
    {
      ...
      fp_offset = AOP (IC_RESULT (ic))->aopu.aop_stk + (AOP (IC_RESULT (ic))->aopu.aop_stk > 0 ? _G.stack.param_offset : 0);
  ...
  // Check if we can do tail call optimization.
           ...
           (!SomethingReturned || IC_RESULT (ic)->aop->size == 1 && aopInReg (IC_RESULT (ic)->aop, 0, IS_GB ? E_IDX : L_IDX)
            || IC_RESULT (ic)->aop->size == 2 && aopInReg (IC_RESULT (ic)->aop, 0, IS_GB ? DE_IDX : HL_IDX)) && !ic->parmBytes
           && !ic->localEscapeAlive && !IFFUNC_ISBANKEDCALL (dtype) && !IFFUNC_ISZ88DK_SHORTCALL (ftype) && _G.omitFramePtr
  ...
  const bool jump = tailjump || !ic->parmBytes && !bigreturn && ic->op != PCALL && !IFFUNC_ISBANKEDCALL (dtype)
                    && !IFFUNC_ISZ88DK_SHORTCALL (ftype) && IFFUNC_ISNORETURN (ftype);
  ...
  if (ic->op == PCALL)
    {
      ...
      if (isLitWord (AOP (IC_LEFT (ic))))
      ...
      else                      // Use bc, since it is the only 16-bit register guarateed to be free even for __z88dk_fastcall with --reserve-regs-iy
        {
          ...
          if (aopInReg (IC_LEFT (ic)->aop, 0, B_IDX) || aopInReg (IC_LEFT (ic)->aop, 0, C_IDX)
              || aopInReg (IC_LEFT (ic)->aop, 1, B_IDX) || aopInReg (IC_LEFT (ic)->aop, 1, C_IDX))
            {
              regalloc_dry_run_cost += 100;
...
genEndFunction (iCode * ic)
{
  ...
  bool is_nmi = (IS_Z80 || IS_Z180 || IS_EZ80_Z80 || IS_Z80N) && IFFUNC_ISCRITICAL (sym->type)
  ...
  else if (size <= 4)
    {
      if (IS_GB && size == 4 && requiresHL (AOP (IC_LEFT (ic))))
      ...
      else
        {
          ...
          for (offset = 0; offset < size; offset++)
            {
              if (requiresHL (AOP (IC_LEFT (ic)))
                  && (ASMOP_RETURN->aopu.aop_reg[offset]->rIdx == H_IDX || ASMOP_RETURN->aopu.aop_reg[offset]->rIdx == L_IDX))
                {
          ...
          for (offset = 0; offset < size; offset++)
            if (skipbytes[offset] && offset + 1 < size && ASMOP_RETURN->aopu.aop_reg[offset]->rIdx == L_IDX
      ...
      if (AOP_TYPE (IC_LEFT (ic)) == AOP_STK || AOP_TYPE (IC_LEFT (ic)) == AOP_EXSTK)
        {
          ...
          fp_offset = AOP (IC_LEFT (ic))->aopu.aop_stk + (AOP (IC_LEFT (ic))->aopu.aop_stk > 0 ? _G.stack.param_offset : 0);
          sp_offset = fp_offset + _G.stack.pushed + _G.stack.offset;
...
genPlus (iCode * ic)
{
  ...
  // eZ80 has lea.
  if (IS_EZ80_Z80 && isPair (IC_RESULT (ic)->aop) && getPairId (IC_LEFT (ic)->aop) == PAIR_IY
  ...
  else if (getPairId (IC_RESULT (ic)->aop) == PAIR_HL && (isPairDead (PAIR_DE, ic) || isPairDead (PAIR_BC, ic))
           && IC_RIGHT (ic)->aop->type == AOP_LIT)
    {
  ...
  if (IC_RESULT (ic)->aop->type == AOP_EXSTK
      && (getPairId (IC_LEFT (ic)->aop) == PAIR_HL || getPairId (IC_RIGHT (ic)->aop) == PAIR_HL) && (isPairDead (PAIR_DE, ic)
          || isPairDead (PAIR_BC,
                         ic))
      && isPairDead (PAIR_HL, ic))
    {
      ...
      goto release;
           ...
           (getPairId (AOP (IC_LEFT (ic))) == PAIR_HL && isPair (AOP (IC_RIGHT (ic)))
            && getPairId (AOP (IC_RIGHT (ic))) != PAIR_IY || getPairId (AOP (IC_RIGHT (ic))) == PAIR_HL
            && isPair (AOP (IC_LEFT (ic))) && getPairId (AOP (IC_LEFT (ic))) != PAIR_IY) && isPairDead (PAIR_HL, ic))
    {
      PAIR_ID pair =
        (getPairId (AOP (IC_LEFT (ic))) == PAIR_HL ? getPairId (AOP (IC_RIGHT (ic))) : getPairId (AOP (IC_LEFT (ic))));
      emit2 ("add hl, %s", _pairs[pair].name);
      _push (PAIR_HL);
      _pop (PAIR_IY);
      goto release;
      ...
      if (AOP_TYPE (IC_LEFT (ic)) == AOP_STK || AOP_TYPE (IC_RIGHT (ic)) == AOP_STK || AOP_TYPE (IC_RESULT (ic)) == AOP_STK)
        {
          if ((AOP_SIZE (IC_LEFT (ic)) == 2 ||
            ...
            {
              if (getPairId (IC_RIGHT (ic)->aop) == PAIR_BC || getPairId (IC_RIGHT (ic)->aop) == PAIR_DE
              ...
              if (getPairId (AOP (IC_LEFT (ic))) == PAIR_BC)
              ...
              else
                {
                  ...
                  if (AOP_TYPE (IC_RIGHT (ic)) == AOP_REG && AOP_SIZE (IC_RIGHT (ic)) == 2 && AOP_TYPE (IC_LEFT (ic)) == AOP_REG
  ...
  if (size == 1 && !aopInReg (leftop, 0, H_IDX) && !aopInReg (leftop, 0, L_IDX) && isPairDead (PAIR_HL, ic))
    {
      if (couldDestroyCarry (AOP (IC_RESULT (ic))) && (AOP (IC_RESULT (ic)) == leftop || AOP (IC_RESULT (ic)) == rightop))
        {
  ...
  for (i = 0, started = FALSE; i < size;)
    {
      ...
      if ((!premoved || i) && !started && i == size - 2 && !i && isPair (AOP (IC_RIGHT (ic)))
          && AOP_TYPE (IC_LEFT (ic)) == AOP_IMMD && getPairId (AOP (IC_RIGHT (ic))) != PAIR_HL && isPairDead (PAIR_HL, ic))
        {
          fetchPair (PAIR_HL, AOP (IC_LEFT (ic)));
          emit2 ("add hl, %s", getPairName (AOP (IC_RIGHT (ic))));
          started = TRUE;
      ...
      else if ((!premoved || i) && !started && i == size - 2 && !i && isPair (AOP (IC_LEFT (ic)))
               && (rightop->type == AOP_LIT || rightop->type == AOP_IMMD) && getPairId (AOP (IC_LEFT (ic))) != PAIR_HL
               && isPairDead (PAIR_HL, ic))
        {
          fetchPair (PAIR_HL, AOP (IC_RIGHT (ic)));
          emit2 ("add hl, %s", getPairName (AOP (IC_LEFT (ic))));
          ...
          i += 2;
               ...
               && isPair (AOP (IC_RIGHT (ic))) && isPairDead (PAIR_HL, ic))
        {
          emit2 ("add hl, %s", getPairName (AOP (IC_RIGHT (ic))));
          started = TRUE;
          regalloc_dry_run_cost += 1;
      ...
      else if ((!premoved || i) && aopInReg (AOP (IC_RESULT (ic)), i, HL_IDX) && aopInReg (leftop, i, HL_IDX)
               && (rightop->type == AOP_LIT && !aopIsLitVal (rightop, i, 1, 0) || rightop->type == AOP_IMMD))
        {
      ...
      // When adding registers the 16 bit addition results in smaller, faster code than an 8-bit addition.
               ...
               && (aopInReg (leftop, i, C_IDX) || aopInReg (rightop, i, C_IDX) || aopInReg (leftop, i, E_IDX)
      ...
      // Skip over this byte.
      else if (!premoved && !started && (leftop->type == AOP_REG || AOP (IC_RESULT (ic))->type == AOP_REG)
      ...
      // Conditional 8-bit inc.
      else if (i == size - 1 && started && aopIsLitVal (rightop, i, 1, 0) && !aopInReg (leftop, i, A_IDX) &&    // adc a, #0 is cheaper than conditional inc.
               (i < leftop->size &&
      ...
      else
        {
          ...
          if (started && (IC_RESULT (ic)->aop->type == AOP_EXSTK || IC_RESULT (ic)->aop->type == AOP_PAIRPTR)
              && requiresHL (IC_RESULT (ic)->aop) && (aopInReg (leftop, i, L_IDX) || aopInReg (leftop, i, H_IDX)
          ...
          if (size &&
              ...
              && (AOP (IC_RESULT (ic))->aopu.aop_reg[i]->rIdx == L_IDX || AOP (IC_RESULT (ic))->aopu.aop_reg[i]->rIdx == H_IDX))
            {
  ...
  if (IS_GB)
    {
      if (left->type == AOP_STK || right->type == AOP_STK || result->type == AOP_STK)
        {
          if ((left->size == 2 || right->size == 2) && (left->size <= 2 && right->size <= 2))
            {
  ...
  while (size)
    {
      if (!IS_GB &&
          ...
          (aopInReg (right, offset, BC_IDX) || aopInReg (right, offset, DE_IDX)
           || ((right->type == AOP_IY || right->type == AOP_HL) && getFreePairId (ic) != PAIR_INVALID)))
        {
          PAIR_ID rightpair;
  ...
  if (IS_RAB && isPairDead (PAIR_HL, ic) && isPairDead (PAIR_BC, ic))
    {
      const bool save_de = (resultsize > 1 && bitVectBitValue (ic->rSurv, D_IDX) ||
                            bitVectBitValue (ic->rSurv, E_IDX) && !(AOP_TYPE (IC_LEFT (ic)) == AOP_REG
      ...
      (byteResult || SPEC_USIGN (getSpec (operandType (IC_LEFT (ic)))) && SPEC_USIGN (getSpec (operandType (IC_RIGHT (ic))))))
    {
      ...
      if (pair == PAIR_INVALID)
        {
          if (!(AOP_TYPE (IC_RESULT (ic)) == AOP_REG &&
                (AOP (IC_RESULT (ic))->aopu.aop_reg[0]->rIdx == L_IDX || AOP (IC_RESULT (ic))->aopu.aop_reg[0]->rIdx == H_IDX ||
                 !byteResult && (AOP (IC_RESULT (ic))->aopu.aop_reg[1]->rIdx == L_IDX
                                 || AOP (IC_RESULT (ic))->aopu.aop_reg[1]->rIdx == H_IDX))))
            {
              pair = PAIR_HL;
                     ...
                     (AOP (IC_RESULT (ic))->aopu.aop_reg[0]->rIdx == E_IDX
                      || AOP (IC_RESULT (ic))->aopu.aop_reg[0]->rIdx == D_IDX || !byteResult
      ...
      if (byteResult && val <= 6 && isPairDead (PAIR_HL, ic) && (isPairDead (PAIR_DE, ic) || isPairDead (PAIR_BC, ic)) &&
          !(AOP_TYPE (IC_RESULT (ic)) == AOP_REG
            && (AOP (IC_RESULT (ic))->aopu.aop_reg[0]->rIdx == E_IDX || AOP (IC_RESULT (ic))->aopu.aop_reg[0]->rIdx == C_IDX)))
        {
          goto no_mlt;
        }
...
no_mlt:
  ...
  pair = PAIR_DE;
  if (getPairId (AOP (IC_LEFT (ic))) == PAIR_BC ||
      (byteResult || !bitVectBitValue (ic->rSurv, B_IDX)) && AOP_TYPE (IC_LEFT (ic)) == AOP_REG
  ...
  else
    {
      ...
      if (ifx && size == 1 && !sign && aopInReg (left->aop, 0, A_IDX) && bitVectBitValue (ic->rSurv, A_IDX) &&
          (AOP_TYPE (right) == AOP_LIT || AOP_TYPE (right) == AOP_REG && AOP (right)->aopu.aop_reg[offset]->rIdx != IYL_IDX
           && AOP (right)->aopu.aop_reg[offset]->rIdx != IYH_IDX || AOP_TYPE (right) == AOP_STK))
        {
          emit3 (A_CP, ASMOP_A, AOP (right));
          result_in_carry = TRUE;
          goto release;
      ...
      if (size > 1 && (IS_GB || IY_RESERVED) && left->aop->type != AOP_REG && right->aop->type != AOP_REG
      ...
      if (AOP_TYPE (right) == AOP_LIT)
        {
          ...
          if (sign)             /* Map signed operands to unsigned ones. This pre-subtraction workaround to lack of signed comparison is cheaper than the post-subtraction one at fix. */
            {
              if (size == 2 && !(IS_GB || !ifx && requiresHL (AOP (result)) && AOP_TYPE (result) != AOP_REG)
                  && isPairDead (PAIR_HL, ic) && (isPairDead (PAIR_DE, ic) || isPairDead (PAIR_BC, ic))
                  && (getPairId (AOP (left)) == PAIR_HL || IS_RAB
      ...
      if (!IS_GB && (!sign || size > 2)
          && (getPartPairId (left->aop, offset) == PAIR_HL || size == 2 && left->aop->type == AOP_IY)
          && isPairDead (PAIR_HL, ic) && (getPartPairId (right->aop, offset) == PAIR_DE
      ...
      while (size)
        {
          if (!IS_GB && (!sign || size > 2) &&
              isPairDead (PAIR_HL, ic) &&
              (getPartPairId (left->aop, offset) == PAIR_HL || left->aop->type == AOP_LIT && right->aop->regs[L_IDX] < offset
               && right->aop->regs[H_IDX] < offset) && (getPartPairId (right->aop, offset) == PAIR_DE
                   || getPartPairId (right->aop, offset) == PAIR_BC))
            {
      ...
      while (size--)
        {
          ...
          if (!byteOfVal (AOP (right)->aopu.aop_lit, offset))
            {
              ...
              a_result = TRUE;
                    ...
                    AOP_TYPE (left) == AOP_REG && AOP (left)->aopu.aop_reg[offset]->rIdx != IYL_IDX
                    && AOP (left)->aopu.aop_reg[offset]->rIdx != IYH_IDX
                    && !bitVectBitValue (ic->rSurv, AOP (left)->aopu.aop_reg[offset]->rIdx))
          ...
          else if (!bitVectBitValue (ic->rSurv, A_IDX) && left->aop->regs[A_IDX] < offset && size
                   && byteOfVal (right->aop->aopu.aop_lit, offset) == 0xff && (left->aop->type == AOP_REG
                       || left->aop->type == AOP_STK)
      ...
      while (size--)
        {
          if (aopInReg (right->aop, offset, A_IDX) || aopInReg (right->aop, offset, HL_IDX)
              || aopInReg (left->aop, offset, BC_IDX) || aopInReg (left->aop, offset, DE_IDX))
            {
              operand *t = right;
              right = left;
              left = t;
          ...
          if (!IS_GB && isPairDead (PAIR_HL, ic) &&
              (aopInReg (left->aop, offset, HL_IDX)
               && (aopInReg (right->aop, offset, BC_IDX) || aopInReg (right->aop, offset, DE_IDX)
                   || getFreePairId (ic) != PAIR_INVALID) || size == 1 && (aopInReg (right->aop, offset, BC_IDX)
...
genAnd (const iCode * ic, iCode * ifx)
{
  ...
  if (aopInReg (right->aop, 0, A_IDX) ||
      !aopInReg (left->aop, 0, A_IDX) && isPair (AOP (right)) && (getPairId (AOP (right)) == PAIR_HL
      ...
      while (sizel)
        {
          ...
          if ((isLiteralBit (bytelit) == 0 || isLiteralBit (bytelit) == 7) && aopInReg (left->aop, 0, A_IDX)
          ...
          /* Testing for the border bits of some 16-bit registers destructively is cheap. */
                   ...
                   (isLiteralBit (bytelit) == 7
                    && (AOP (left)->aopu.aop_reg[offset]->rIdx == H_IDX && isPairDead (PAIR_HL, ic) || IS_RAB
                        && AOP (left)->aopu.aop_reg[offset]->rIdx == D_IDX && isPairDead (PAIR_DE, ic)
                        || AOP (left)->aopu.aop_reg[offset]->rIdx == IYH_IDX && isPairDead (PAIR_IY, ic))
                    || isLiteralBit (bytelit) == 0 && IS_RAB && (AOP (left)->aopu.aop_reg[offset]->rIdx == L_IDX
                        && isPairDead (PAIR_HL, ic)
          ...
          /* Non-destructive and when exactly one bit per byte is set. */
                   ...
                   (AOP_TYPE (left) == AOP_STK || aopInReg (left->aop, 0, A_IDX) || AOP_TYPE (left) == AOP_HL
                    || AOP_TYPE (left) == AOP_IY || AOP_TYPE (left) == AOP_REG && AOP (left)->aopu.aop_reg[0]->rIdx != IYL_IDX))
            {
              if (!regalloc_dry_run)
                {
          ...
          else if ((IS_Z180 || IS_EZ80_Z80 || IS_Z80N) && aopInReg (left->aop, 0, A_IDX) && bitVectBitValue (ic->rSurv, A_IDX)
                   && bytelit != 0x0ff)
            {
              if (!regalloc_dry_run)
                {
  ...
  for (int i = 0; i < size;)
    {
      if (!bitVectBitValue (ic->rSurv, A_IDX) && left->aop->regs[A_IDX] <= i && right->aop->regs[A_IDX] <= i
          ...
          if (bytelit == 0x00)
          ...
          else if (bytelit == 0xff)
            {
              ...
              continue;
                   ...
                   (AOP_TYPE (result) == AOP_REG || left == right
                    && (AOP_TYPE (result) == AOP_STK || AOP_TYPE (result) == AOP_DIR)))
            {
              ...
              continue;
                   ...
                   (aopInReg (result->aop, i, HL_IDX)
                    && (aopInReg (left->aop, i, HL_IDX) && !isPairInUse (PAIR_DE, ic) || aopInReg (left->aop, i, DE_IDX))
                    || aopInReg (result->aop, i, H_IDX) && aopInReg (result->aop, i + 1, L_IDX)
                    && (aopInReg (left->aop, i, H_IDX) && aopInReg (left->aop, i + 1, L_IDX) && !isPairInUse (PAIR_DE, ic)
                        || aopInReg (left->aop, i, D_IDX) && aopInReg (left->aop, i + 1, E_IDX))))
            {
              unsigned short mask =
                aopInReg (result->aop, i,
                          L_IDX) ? (bytelit +
                                    (byteOfVal (right->aop->aopu.aop_lit, i + 1) << 8)) : (byteOfVal (right->aop->aopu.aop_lit,
                                        i + 1) + (bytelit << 8));
      ...
      if (IS_RAB)
        {
          const bool this_byte_l = aopInReg (result->aop, i, L_IDX) &&
                                   (aopInReg (left->aop, i, L_IDX) && aopInReg (left->aop, i, E_IDX) || aopInReg (left->aop, i, E_IDX)
                                    && aopInReg (left->aop, i, L_IDX));
          const bool this_byte_h = aopInReg (result->aop, i, H_IDX) && (aopInReg (left->aop, i, H_IDX)
                                   && aopInReg (left->aop, i, D_IDX)
                                   || aopInReg (left->aop, i, D_IDX)
                                   && aopInReg (left->aop, i, H_IDX));
          const bool next_byte_l = aopInReg (result->aop, i + 1, L_IDX) && (aopInReg (left->aop, i + 1, L_IDX)
                                   && aopInReg (left->aop, i + 1, E_IDX)
                                   || aopInReg (left->aop, i + 1, E_IDX)
                                   && aopInReg (left->aop, i + 1, L_IDX));
          const bool next_byte_h = aopInReg (result->aop, i + 1, H_IDX) && (aopInReg (left->aop, i + 1, H_IDX)
...
shiftR2Left2Result (const iCode * ic, operand * left, int offl, operand * result, int offr, int shCount, int is_signed)
{
  ...
  if (IS_RAB && !is_signed && shCount < 4 && (getPairId (AOP (result)) == PAIR_HL || getPairId (AOP (result)) == PAIR_DE))
    {
...
shiftL2Left2Result (operand * left, operand * result, int shCount, const iCode * ic)
{
  ...
  if ((result->aop->type == AOP_HL || result->aop->type == AOP_IY) && (left->aop->type == AOP_HL || left->aop->type == AOP_IY) && !IS_GB && isPairDead (PAIR_HL, ic) && (shCount > 1 || !sameRegs (result->aop, left->aop)))    // Being able to use cheap add hl, hl is worth it in most cases.
    {
      shiftaop = ASMOP_HL;
      fetchPairLong (PAIR_HL, left->aop, ic, 0);
    }
  else if (AOP_TYPE (result) != AOP_REG && AOP_TYPE (left) == AOP_REG && AOP_SIZE (left) >= 2
           && !bitVectBitValue (ic->rSurv, AOP (left)->aopu.aop_reg[0]->rIdx)
           && !bitVectBitValue (ic->rSurv, AOP (left)->aopu.aop_reg[1]->rIdx) || getPairId (AOP (left)) == PAIR_HL
           && isPairDead (PAIR_HL, ic))
    {
      shiftaop = left->aop;
    }
  ...
  if (shCount == 0)
  ...
  else
    {
      ...
      bool use_b = (!IS_GB && !bitVectBitValue (ic->rSurv, B_IDX)
                    && (shiftaop->type != AOP_REG || shiftaop->aopu.aop_reg[0]->rIdx != B_IDX
...
shiftL1Left2Result (operand * left, int offl, operand * result, int offr, unsigned int shCount, const iCode * ic)
{
  ...
  if (sameRegs (result->aop, left->aop) && aopInReg (result->aop, offr, L_IDX) && isPairDead (PAIR_HL, ic)
      && !optimize.codeSpeed && offr == offl)
    {
      while (shCount--)
        {
          emit2 ("add hl, hl");
      ...
      while (shCount--)
        {
          emit3 (A_SLA, AOP (result), 0);
           ...
           (!IS_Z80N && aopInReg (result->aop, offr, C_IDX) && isPairDead (PAIR_BC, ic) || aopInReg (result->aop, offr, E_IDX)
            && isPairDead (PAIR_DE, ic) || !IS_Z80N && aopInReg (result->aop, offr, L_IDX) && isPairDead (PAIR_HL, ic)))
    {
      ...
      if (!top)
        cheapMove (pair == PAIR_BC ? ASMOP_C : (pair == PAIR_DE ? ASMOP_E : ASMOP_L), 0, left->aop, offl,
                   !bitVectBitValue (ic->rSurv, A_IDX));
...
genLeftShiftLiteral (operand * left, operand * right, operand * result, const iCode * ic)
{
  ...
  if (shCount >= (size * 8))
    genMove (result->aop, ASMOP_ZERO, !bitVectBitValue (ic->rSurv, A_IDX), isPairDead (PAIR_HL, ic)
             && left->aop->regs[H_IDX] < 0 && left->aop->regs[L_IDX] < 0);
  else
    {
      switch (size)
        {
...
genLeftShift (const iCode * ic)
{
  ...
  if (AOP_TYPE (right) == AOP_REG && !bitVectBitValue (ic->rSurv, AOP (right)->aopu.aop_reg[0]->rIdx)
      && AOP (right)->aopu.aop_reg[0]->rIdx != IYL_IDX && (sameRegs (AOP (left), AOP (result)) || AOP_TYPE (left) != AOP_REG)
      && (AOP_TYPE (result) != AOP_REG || AOP (result)->aopu.aop_reg[0]->rIdx != AOP (right)->aopu.aop_reg[0]->rIdx
          ...
          && !aopInReg (AOP (result), 3, AOP (right)->aopu.aop_reg[0]->rIdx)))
    {
      countreg = AOP (right)->aopu.aop_reg[0]->rIdx;
           ...
           && (sameRegs (AOP (left), AOP (result)) || AOP_TYPE (left) != AOP_REG || shift_by_lit)
           && !aopInReg (AOP (result), 0, B_IDX) && !aopInReg (AOP (result), 1, B_IDX) && !aopInReg (AOP (result), 2, B_IDX)
           && !aopInReg (AOP (result), 3, B_IDX))
    {
      countreg = B_IDX;
    }
  else
  ...
  save_a = (countreg == A_IDX && !shift_by_lit) &&
           !(AOP_TYPE (left) == AOP_REG && AOP_TYPE (result) != AOP_REG ||
             !IS_GB && (AOP_TYPE (left) == AOP_STK && canAssignToPtr3 (result->aop) || AOP_TYPE (result) == AOP_STK
                        && canAssignToPtr3 (left->aop)));
  ...
  while (size)
    {
      if (size >= 2 && offset + 1 >= byteshift &&
          AOP_TYPE (result) == AOP_REG &&
          (getPartPairId (AOP (result), offset) == PAIR_HL || IS_RAB && getPartPairId (AOP (result), offset) == PAIR_DE))
        {
...
genrshOne (operand * result, operand * left, int shCount, int is_signed, const iCode * ic)
{
  ...
  if ((IS_Z180 || IS_EZ80_Z80 || IS_Z80N) && !is_signed && shCount >= 3 && shCount <= 6 + a_dead &&     // Try to use mlt.
      (!IS_Z80N && aopInReg (result->aop, 0, B_IDX) && isPairDead (PAIR_BC, ic) || aopInReg (result->aop, 0, D_IDX)
       && isPairDead (PAIR_DE, ic) || !IS_Z80N && aopInReg (result->aop, 0, H_IDX) && isPairDead (PAIR_HL, ic)))
    {
  ...
  if (AOP_TYPE (right) == AOP_REG && !bitVectBitValue (ic->rSurv, AOP (right)->aopu.aop_reg[0]->rIdx)
                  ...
                  && (AOP_SIZE (result) < 4 || AOP (result)->aopu.aop_reg[3]->rIdx != AOP (right)->aopu.aop_reg[0]->rIdx)))))
    {
      countreg = AOP (right)->aopu.aop_reg[0]->rIdx;
           ...
           && (sameRegs (AOP (left), AOP (result)) || AOP_TYPE (left) != AOP_REG || shift_by_lit)
           && (AOP_TYPE (result) != AOP_REG || AOP (result)->aopu.aop_reg[0]->rIdx != B_IDX
...
static void
offsetPair (PAIR_ID pair, PAIR_ID extrapair, bool save_extrapair, int val)
{
...
genPointerGet (const iCode * ic)
{
  ...
  else if (!IS_GB && AOP_TYPE (left) == AOP_IMMD && getPartPairId (AOP (result), 0) != PAIR_INVALID
  ...
  if (!IS_GB && (getPairId (AOP (left)) == PAIR_BC || getPairId (AOP (left)) == PAIR_DE) && AOP_TYPE (result) == AOP_STK
      && !rightval || getPairId (AOP (left)) == PAIR_IY && SPEC_BLEN (getSpec (operandType (result))) < 8 && rightval_in_range)
    {
  ...
  if (isPair (AOP (result)) && IS_EZ80_Z80 && getPairId (AOP (left)) == PAIR_HL && !IS_BITVAR (retype) && !rightval)
    {
      ...
      goto release;
           ...
           && (getPairId (AOP (result)) == PAIR_HL || size == 2
               && (aopInReg (result->aop, 0, L_IDX) || aopInReg (result->aop, 0, H_IDX))))
    {
  ...
  if (pair == PAIR_HL
      || (!IS_GB && (getPairId (AOP (left)) == PAIR_BC || getPairId (AOP (left)) == PAIR_DE) && AOP_TYPE (result) == AOP_STK))
    {
      size = AOP_SIZE (result);
      offset = 0;
      int last_offset = 0;
  ...
  if (blen < 8)
    {
      ...
      else if (blen == 4 && bstr % 4 == 0 && pair == PAIR_HL && !aopInReg (right->aop, 0, A_IDX) && !requiresHL (right->aop)
...
genPointerSet (iCode * ic)
{
  ...
  if (!IS_GB && !isBitvar && isLitWord (AOP (result)) && size == 4 && offset == 0 &&
      (getPartPairId (AOP (right), 0) != PAIR_INVALID && getPartPairId (AOP (right), 2) != PAIR_INVALID
       || isLitWord (AOP (right))))
    {
      if (isLitWord (AOP (right)))
        {
  ...
  if (IS_BOOL (operandType (cond)) && !aopInReg (cond->aop, 0, A_IDX))
    {
      ...
      goto release;
           ...
           (aopInReg (cond->aop, 0, B_IDX) || aopInReg (cond->aop, 0, C_IDX) || aopInReg (cond->aop, 0, D_IDX)
            || aopInReg (cond->aop, 0, E_IDX) || aopInReg (cond->aop, 0, H_IDX) || aopInReg (cond->aop, 0, L_IDX)))
    {
...
genAssign (const iCode * ic)
{
  ...
  if (isPair (AOP (result)))
  ...
  else if (size == 2 && isPairDead (PAIR_HL, ic) && (!IS_GB && (AOP_TYPE (right) == AOP_STK && !_G.omitFramePtr || AOP_TYPE (right) == AOP_IY || AOP_TYPE (right) == AOP_LIT) && AOP_TYPE (result) == AOP_IY || // Use ld (nn), hl
  ...
  else if (size == 2 && getPairId (AOP (right)) != PAIR_INVALID && getPairId (AOP (right)) != PAIR_IY
           && AOP_TYPE (result) != AOP_REG)
    {
  ...
  else if (size == 2 && requiresHL (AOP (right)) && requiresHL (AOP (result)) && isPairDead (PAIR_DE, ic)
           && (IS_GB /*|| IY_RESERVED */ ))
    {
  ...
  else if (size == 4 && (requiresHL (right->aop) && right->aop->type != AOP_REG)
           && (requiresHL (result->aop) && result->aop->type != AOP_REG) && isPairDead (PAIR_DE, ic) && (IS_GB || IY_RESERVED))
    {
  ...
  else
    {
          ...
          if (l_better)
            {
              ...
              if (AOP_TYPE (result) == AOP_STK || AOP_TYPE (result) == AOP_EXSTK)
                {
                  int fp_offset =
                    AOP (result)->aopu.aop_stk + offset + (AOP (result)->aopu.aop_stk > 0 ? _G.stack.param_offset : 0);
                  int sp_offset = fp_offset + _G.stack.pushed + _G.stack.offset;
                  emit2 ("ld hl, !immed%d", sp_offset);
                  emit2 ("add hl, sp");
                  emit2 ("ex de, hl");
              ...
              if (AOP_TYPE (right) == AOP_STK || AOP_TYPE (right) == AOP_EXSTK)
                {
                  int fp_offset =
                    AOP (right)->aopu.aop_stk + offset + (AOP (right)->aopu.aop_stk > 0 ? _G.stack.param_offset : 0);
                  int sp_offset = fp_offset + _G.stack.pushed + _G.stack.offset;
                  emit2 ("ld hl, !immed%d", sp_offset);
                  emit2 ("add hl, sp");
                  regalloc_dry_run_cost += 4;
      ...
      if ((result->aop->type == AOP_REG || result->aop->type == AOP_STK || result->aop->type == AOP_EXSTK)
          && (right->aop->type == AOP_REG || right->aop->type == AOP_STK || right->aop->type == AOP_LIT
        ...
        while (size--)
          {
            const bool hl_free = hl_dead &&
                                 ...
                                 (result->aop->regs[L_IDX] < 0 || result->aop->regs[L_IDX] >= offset) && (result->aop->regs[H_IDX] < 0
...
genCast (const iCode * ic)
{
  ...
  if (IS_BOOL (operandType (result)) && IS_RAB && right->aop->size == 2 &&
      (aopInReg (right->aop, 0, HL_IDX) && isPairDead (PAIR_HL, ic) || aopInReg (right->aop, 0, IY_IDX)
       && isPairDead (PAIR_IY, ic)))
    {
...
setupForMemcpy (const iCode * ic, const operand * to, const operand * from, const operand * count)
{
  ...
  if (AOP_TYPE (to) == AOP_REG && AOP_TYPE (from) == AOP_REG)
  ...
  else
    {
      ...
      if (AOP_TYPE (from) != AOP_REG || AOP (from)->aopu.aop_reg[0]->rIdx != E_IDX && AOP (from)->aopu.aop_reg[0]->rIdx != D_IDX
          && AOP (from)->aopu.aop_reg[1]->rIdx != E_IDX && AOP (from)->aopu.aop_reg[1]->rIdx != D_IDX)
        {
          fetchPair (PAIR_DE, AOP (to));
          fetchPair (PAIR_HL, AOP (from));
        }
      /* HL is free. Write it first. */
      else if (AOP_TYPE (to) != AOP_REG || AOP (to)->aopu.aop_reg[0]->rIdx != L_IDX && AOP (to)->aopu.aop_reg[0]->rIdx != H_IDX
               && AOP (to)->aopu.aop_reg[1]->rIdx != L_IDX && AOP (to)->aopu.aop_reg[1]->rIdx != H_IDX)
        {
          fetchPair (PAIR_HL, AOP (from));
          fetchPair (PAIR_DE, AOP (to));
        }
      /* L is free, but H is not. */
               ...
               && (AOP_TYPE (from) != AOP_REG || AOP (from)->aopu.aop_reg[0]->rIdx != L_IDX
  ...
  if (AOP_TYPE (dst) == AOP_REG && !direct_c && AOP_TYPE (c) == AOP_REG)
    {
      ...
      short oparray[2];
      bool early_a = AOP_TYPE (c) == AOP_REG && (AOP (c)->aopu.aop_reg[0]->rIdx == L_IDX
...
genBuiltInMemset (const iCode * ic, int nParams, operand ** pparams)
{
  ...
  bool double_loop;
  unsigned size;
  bool live_BC = !isPairDead (PAIR_BC, ic), live_DE = !isPairDead (PAIR_DE, ic), live_HL = !isPairDead (PAIR_HL, ic), live_B =
...
genBuiltIn (iCode * ic)
{
  ...
  wassertl (!ic->prev || ic->prev->op != SEND
            || !ic->prev->builtinSEND, "genBuiltIn() must be called on first SEND icode only.");
