...
template < class G_t, class I_t > float
default_operand_cost (const operand * o, const assignment & a, unsigned short int i, const G_t & G, const I_t & I)
{
  float c = 0.0f;
              ...
              while (++oi != oi_end)
                {
                  v = oi->second;
                  c +=
                    (std::binary_search (a.local.begin (), a.local.end (), v) ? 1.0f : std::numeric_limits <
                     float >::infinity ());
                  byteregs[I[v].byte] = a.global[v];
                  size++;
                }
              ...
              while (++oi != oi_end)
                {
                  v = oi->second;
                  c +=
                    (!std::binary_search (a.local.begin (), a.local.end (), v) ? 4.0f : std::numeric_limits <
                     float >::infinity ());
                }
            }
        }
...
template < class G_t > static bool
operand_in_reg (const operand * o, const i_assignment_t & ia, unsigned short int i, const G_t & G)
{
  if (!o || !IS_SYMOP (o))
    {
      return (false);
...
// Return true, iff the operand is placed in a reg.
template < class G_t > static bool
operand_byte_in_reg (const operand * o, int offset, reg_t r, const assignment & a, unsigned short int i, const G_t & G)
{
  if (!o || !IS_SYMOP (o))
    {
      return (false);
...
template < class G_t > bool
operand_on_stack (const operand * o, const assignment & a, unsigned short int i, const G_t & G)
{
  if (!o || !IS_SYMOP (o))
    {
      return (false);
...
template < class G_t > static bool
operand_is_pair (const operand * o, const assignment & a, unsigned short int i, const G_t & G)
{
  if (!o || !IS_SYMOP (o))
    {
      return (false);
...
Ainst_ok (const assignment & a, unsigned short int i, const G_t & G, const I_t & I)
{
  ...
  if ((ic->op == EQ_OP || ic->op == '<' && SPEC_USIGN (getSpec (operandType (left)))
       && SPEC_USIGN (getSpec (operandType (right)))) && getSize (operandType (IC_LEFT (ic))) == 1
      && ifxForOp (IC_RESULT (ic), ic) && operand_in_reg (left, REG_A, ia, i, G) && (IS_OP_LITERAL (right)
          ...
          || operand_in_reg (right, REG_L, ia, i,
                             G)))
    {
      return (true);
    }
  ...
  const bool dying_A = result_in_A || dying.find (ia.registers[REG_A][1]) != dying.end ()
  ...
  if (ic->op == IFX && dying_A && (getSize (operandType (left)) >= 1 &&
                                   operand_byte_in_reg (left, 0, REG_A, a, i, G) || getSize (operandType (left)) >= 2
                                   && !IS_FLOAT (operandType (left)) && operand_byte_in_reg (left, 1, REG_A, a, i, G)))
    {
      return (true);
    }
  ...
  if (ic->op == IFX && getSize (operandType (left)) == 1 &&
      (operand_byte_in_reg (left, 0, REG_B, a, i, G) || operand_byte_in_reg (left, 0, REG_C, a, i, G)
       || operand_byte_in_reg (left, 0, REG_D, a, i, G) || operand_byte_in_reg (left, 0, REG_E, a, i, G)
       || operand_byte_in_reg (left, 0, REG_H, a, i, G) || operand_byte_in_reg (left, 0, REG_L, a, i, G)))
    {
      return (true);
    }
  ...
  if (ic->op == CAST && getSize (operandType (IC_RESULT (ic))) == 1 &&
      (operand_in_reg (result, REG_B, ia, i, G) || operand_in_reg (result, REG_C, ia, i, G)
       || operand_in_reg (result, REG_D, ia, i, G) || operand_in_reg (result, REG_E, ia, i, G)
       || operand_in_reg (result, REG_A, ia, i, G)) && (operand_in_reg (result, REG_B, ia, i, G)
  ...
  if (ic->op == CAST && operand_in_reg (right, REG_A, ia, i, G) &&
      !operand_in_reg (result, REG_A, ia, i, G) && (operand_in_reg (result, REG_C, ia, i, G)
          || operand_in_reg (result, REG_E, ia, i, G)
  ...
  if ((ic->op == GET_VALUE_AT_ADDRESS || ic->op == CAST && !operand_in_reg (right, REG_A, ia, i, G)) && IS_ITEMP (result)
      && operand_byte_in_reg (result, getSize (operandType (IC_RESULT (ic))) - 1, REG_A, a, i, G))
    {
      return (true);
    }

  if (ic->op == LEFT_OP && getSize (operandType (IC_RESULT (ic))) == 2 && IS_OP_LITERAL (right)
      && byteOfVal (OP_VALUE (IC_RIGHT (ic)), 0) == 7)
    {
  ...
  if ((ic->op == '+' || ic->op == '-') && IS_OP_LITERAL (right) && ulFromVal (OP_VALUE (IC_RIGHT (ic))) <= 2 &&
      (getSize (operandType (IC_RESULT (ic))) == 2 && operand_is_pair (IC_RESULT (ic), a, i, G)
       || getSize (operandType (IC_RESULT (ic))) == 1 && operand_in_reg (result, ia, i, G)
  ...
  if (ic->op == GET_VALUE_AT_ADDRESS)   // Any register can be assigned from (hl) and (iy), so we don't need to go through a then.
    return (!IS_BITVAR (getSpec (operandType (result))) &&
            (getSize (operandType (result)) == 1 || operand_is_pair (left, a, i, G)
             && (operand_in_reg (left, REG_L, ia, i, G) && !ulFromVal (OP_VALUE (IC_RIGHT (ic)))
                 || operand_in_reg (left, REG_IYL, ia, i, G) && ulFromVal (OP_VALUE (IC_RIGHT (ic))) <= 127)));
  ...
  if (ic->op == '=' && POINTER_SET (ic) &&      // Any register can be assigned to (hl) and (iy), so we don't need to go through a then.
      ...
      (getSize (operandType (right)) == 1 || operand_is_pair (result, a, i, G)
       && (operand_in_reg (result, REG_L, ia, i, G) || operand_in_reg (result, REG_IYL, ia, i, G))))
    {
      return (true);
    }
  ...
  if (1)
    {
      ...
      if (ic->op == '+' && IS_ITEMP (left) && IS_ITEMP (IC_RESULT (ic)) && IS_OP_LITERAL (right) &&
          ulFromVal (OP_VALUE (IC_RIGHT (ic))) == 1 && OP_KEY (IC_RESULT (ic)) == OP_KEY (IC_LEFT (ic)))
        {
          return (true);
        }
      ...
      if ((ic->op == '=' || ic->op == CAST) && !POINTER_SET (ic) && !(ic->op == CAST && IS_BOOL (operandType (result))) &&
          (operand_in_reg (right, REG_A, ia, i, G) || operand_in_reg (right, REG_B, ia, i, G)
           || operand_in_reg (right, REG_C, ia, i, G) || operand_in_reg (right, REG_D, ia, i, G)
           || operand_in_reg (right, REG_E, ia, i, G) || operand_in_reg (right, REG_H, ia, i, G)
           || operand_in_reg (right, REG_L, ia, i, G)) && (operand_in_reg (right, REG_A, ia, i, G)
  ...
  if (input_in_A && dying_A)
    {
      if (ic->op != IFX &&
          ...
          !((ic->op == RIGHT_OP || ic->op == LEFT_OP) &&
            (IS_OP_LITERAL (right) || operand_in_reg (right, REG_A, ia, i, G) || getSize (operandType (IC_RESULT (ic))) == 1
             && ia.registers[REG_B][1] < 0)) && !((ic->op == '=' || ic->op == CAST) && !(IY_RESERVED && POINTER_SET (ic)))
          && !IS_BITWISE_OP (ic) && !(ic->op == '~') && !(ic->op == '*'
  ...
  if (result_in_A &&
      ...
      ic->op != CAST &&
      ic->op != CALL &&
      ic->op != PCALL && ic->op != GETHBIT && !((ic->op == LEFT_OP || ic->op == RIGHT_OP) && IS_OP_LITERAL (right)))
    {
...
template < class G_t, class I_t > static bool
HLinst_ok (const assignment & a, unsigned short int i, const G_t & G, const I_t & I)
{
  const iCode *ic = G[i].ic;
...
#if 0
  if (ic->key == 3)
    std::cout << "HLinst_ok: at (" << i << ", " << ic->key << ")\nL = (" << ia.registers[REG_L][0] << ", " << ia.
              registers[REG_L][1] << "), H = (" << ia.registers[REG_H][0] << ", " << ia.
              registers[REG_H][1] << ")inst " << i << ", " << ic->key << "\n";
#endif
  ...
  bool dying_L = result_in_L || dying.find (ia.registers[REG_L][1]) != dying.end ()
                 || dying.find (ia.registers[REG_L][0]) != dying.end ();
  bool dying_H = result_in_H || dying.find (ia.registers[REG_H][1]) != dying.end ()
  ...
  if (IS_GB && ic->op == IPUSH && !operand_is_pair (left, a, i, G) && ia.registers[REG_A][1] >= 0 &&
      !(getSize (operandType (left)) == 1
        && (operand_in_reg (left, REG_A, ia, i, G) || operand_in_reg (left, REG_B, ia, i, G)
            || operand_in_reg (left, REG_D, ia, i, G) || operand_in_reg (left, REG_H, ia, i, G))))
    {
      return (false);
    }
  ...
  if (IS_GB && (operand_on_stack (result, a, i, G) || operand_on_stack (left, a, i, G) || operand_on_stack (right, a, i, G)) && (ic->op == RIGHT_OP || ic->op == LEFT_OP || ic->op == '=' || ic->op == CAST || ic->op == '-' || ic->op == UNARYMINUS || ic->op == BITWISEAND || ic->op == '|') && !(result_only_HL && getSize (operandType (result)) == 1))     // Size of result needs to be checked after checking ic->op to esnure that there is a result operand.
    {
      return (false);
    }
  ...
  if (ic->op == '+' && getSize (operandType (result)) >= 2 && input_in_HL &&
      ((exstk ? operand_on_stack (left, a, i, G) : IS_TRUE_SYMOP (left))
       ...
       || (exstk ? operand_on_stack (right, a, i, G) : IS_TRUE_SYMOP (right)) && (ia.registers[REG_L][1] > 0
  ...
  if (ic->op == '+' && getSize (operandType (result)) == 2 &&
      (IS_OP_LITERAL (right) && ulFromVal (OP_VALUE (IC_RIGHT (ic))) <= 3 || IS_OP_LITERAL (left)
       && ulFromVal (OP_VALUE (IC_LEFT (ic))) <= 3) && (operand_in_reg (result, REG_L, ia, i, G)
  ...
  if (!IS_GB && ic->op == '+' && getSize (operandType (result)) == 2 && !IS_TRUE_SYMOP (result) && (result_only_HL || operand_in_reg (result, REG_IYL, ia, i, G) && operand_in_reg (result, REG_IYH, ia, i, G)) && (ia.registers[REG_C][1] < 0 && ia.registers[REG_B][1] < 0 || ia.registers[REG_E][1] < 0 && ia.registers[REG_D][1] < 0))      // Can use ld rr, (nn) instead of (hl).
    {
      return (true);
    }

  if (!IS_GB && ic->op == '+' && getSize (operandType (result)) == 2 && IS_TRUE_SYMOP (left) && (IS_OP_LITERAL (right) && ulFromVal (OP_VALUE (IC_RIGHT (ic))) <= 3 || IS_OP_LITERAL (left) && ulFromVal (OP_VALUE (IC_LEFT (ic))) <= 3) && (operand_in_reg (result, REG_C, ia, i, G) && I[ia.registers[REG_C][1]].byte == 0 && operand_in_reg (result, REG_B, ia, i, G) || operand_in_reg (result, REG_E, ia, i, G) && I[ia.registers[REG_E][1]].byte == 0 && operand_in_reg (result, REG_D, ia, i, G)))       // Can use ld rr, (nn) followed by inc rr
    {
      return (true);
    }

  if ((ic->op == '+' || ic->op == '-' || ic->op == UNARYMINUS) && getSize (operandType (result)) >= 2 && (IS_TRUE_SYMOP (result) && !operand_on_stack (result, a, i, G) || (operand_on_stack (left, a, i, G) ? exstk : IS_TRUE_SYMOP (left)) || (operand_on_stack (right, a, i, G) ? exstk : IS_TRUE_SYMOP (right))))   // Might use (hl).
    {
      return (false);
    }
  ...
  if (result_only_HL && !POINTER_SET (ic) &&
      (ic->op == ADDRESS_OF ||
       ic->op == GET_VALUE_AT_ADDRESS || ic->op == '+' || ic->op == '*' || ic->op == '=' || ic->op == CAST))
    {
      return (true);
    }

  if (!exstk && !isOperandInDirSpace (IC_LEFT (ic)) && !isOperandInDirSpace (IC_RIGHT (ic))
      && !isOperandInDirSpace (IC_RESULT (ic)) && (ic->op == '-' || ic->op == UNARYMINUS || ic->op == '~' || ic->op == '<'
          || ic->op == '>'))
    {
      return (true);
    }
  ...
  if ((ic->op == LEFT_OP || ic->op == RIGHT_OP)
      && (getSize (operandType (result)) <= 1 || !IS_TRUE_SYMOP (result) || !(IS_GB || IY_RESERVED)) && (!exstk
          ||
          ((!operand_on_stack
            (left, a, i, G)
            || !input_in_HL
  ...
  if (!IS_GB && ic->op == ADDRESS_OF &&
      (operand_in_reg (result, REG_IYL, ia, i, G) && ia.registers[REG_IYL][1] > 0 && I[ia.registers[REG_IYL][1]].byte == 0
       && operand_in_reg (result, REG_IYH, ia, i, G) || !OP_SYMBOL_CONST (left)->onStack
       && operand_in_reg (result, REG_C, ia, i, G) && ia.registers[REG_C][1] > 0 && I[ia.registers[REG_C][1]].byte == 0
       && operand_in_reg (result, REG_B, ia, i, G) || !OP_SYMBOL_CONST (left)->onStack
       && operand_in_reg (result, REG_E, ia, i, G) && ia.registers[REG_E][1] > 0 && I[ia.registers[REG_E][1]].byte == 0
       && operand_in_reg (result, REG_D, ia, i, G)))
    {
      return (true);
    }
  ...
  if (exstk && !result_only_HL
      && (operand_on_stack (left, a, i, G) || operand_on_stack (right, a, i, G) || operand_on_stack (result, a, i, G))
      && ic->op == '+')
    {
      return (false);
    }
  ...
  if ((!POINTER_SET (ic) && !POINTER_GET (ic) && ((ic->op == '=' ||
       ic->op == CAST || ic->op == UNARYMINUS || ic->op == RIGHT_OP ||
       /*ic->op == '-' || */
       IS_BITWISE_OP (ic) ||
       /*ic->op == '>' ||
          ic->op == '<' ||
          ic->op == EQ_OP || */
       (ic->op == '+' && getSize (operandType (IC_RESULT (ic))) == 1) || (ic->op == '+' && getSize (operandType (IC_RESULT (ic))) <= 2 && (result_only_HL || !IS_GB))))))   // 16 bit addition on gbz80 might need to use add hl, rr.
    {
      return (true);
    }
  ...
  if (POINTER_GET (ic) && getSize (operandType (IC_RESULT (ic))) == 1 && !IS_BITVAR (getSpec (operandType (result))) && (operand_in_reg (right, REG_C, ia, i, G) && I[ia.registers[REG_C][1]].byte == 0 && operand_in_reg (right, REG_B, ia, i, G) ||   // Uses ld a, (bc)
  ...
  if (ic->key == 6)
    {
      std::cout << "HLinst_ok: L = (" << ia.registers[REG_L][0] << ", " << ia.registers[REG_L][1] << "), H = (" << ia.
                registers[REG_H][0] << ", " << ia.registers[REG_H][1] << ")inst " << i << ", " << ic->key << "\n";
      std::cout << "Result in L: " << result_in_L << ", result in H: " << result_in_H << "\n";
...
template < class G_t, class I_t > static bool
IYinst_ok (const assignment & a, unsigned short int i, const G_t & G, const I_t & I)
{
  const iCode *ic = G[i].ic;
  ...
  if (!result_in_IY && !input_in_IY &&
      ...
      !(IC_RIGHT (ic) && IS_TRUE_SYMOP (IC_RIGHT (ic))) && !(IC_LEFT (ic) && IS_TRUE_SYMOP (IC_LEFT (ic))))
    {
      return (true);
    }
  ...
  if (ic->op == IPUSH &&
      ...
      (I[ia.registers[REG_IYL][1]].byte == 0 && I[ia.registers[REG_IYH][1]].byte == 1 || I[ia.registers[REG_IYL][1]].byte == 2
  ...
  if (!unused_IYL && I[ia.registers[REG_IYL][1]].size != 2 || !unused_IYH && I[ia.registers[REG_IYH][1]].size != 2 ||
      ia.registers[REG_IYL][0] >= 0 && I[ia.registers[REG_IYL][0]].size != 2 || ia.registers[REG_IYH][0] >= 0
      && I[ia.registers[REG_IYH][0]].size != 2)
    {
      return (false);
    }
  if (ia.registers[REG_IYL][1] >= 0
      && (ia.registers[REG_IYH][1] <= 0 || I[ia.registers[REG_IYL][1]].v != I[ia.registers[REG_IYH][1]].v))
    {
      return (false);
    }
  if (ia.registers[REG_IYH][1] >= 0
      && (ia.registers[REG_IYL][1] <= 0 || I[ia.registers[REG_IYH][1]].v != I[ia.registers[REG_IYL][1]].v))
    {
      return (false);
    }
  if (ia.registers[REG_IYL][0] >= 0
      && (ia.registers[REG_IYH][0] <= 0 || I[ia.registers[REG_IYL][0]].v != I[ia.registers[REG_IYH][0]].v))
    {
      return (false);
    }
  if (ia.registers[REG_IYH][0] >= 0
      && (ia.registers[REG_IYL][0] <= 0 || I[ia.registers[REG_IYH][0]].v != I[ia.registers[REG_IYL][0]].v))
    {
      return (false);
    }
  ...
  if (ia.registers[REG_IYL][0] >= 0 && I[ia.registers[REG_IYL][0]].byte != 0 || ia.registers[REG_IYH][0] >= 0
  ...
  if (ic->key == 32)
    {
      ...
      //print_assignment(a);
      std::cout << "\n";
      std::cout << "2IYinst_ok: at (" << i << ", " << ic->key << ")\nIYL = (" << ia.registers[REG_IYL][0] << ", " << ia.
                registers[REG_IYL][1] << "), IYH = (" << ia.registers[REG_IYH][0] << ", " << ia.
                registers[REG_IYH][1] << ")inst " << i << ", " << ic->key << "\n";
    }
#endif

  if (result_in_IY && (ic->op == '=' && !(POINTER_SET (ic) && isOperandInDirSpace (IC_RIGHT (ic))) || ic->op == CAST && getSize (operandType (IC_RESULT (ic))) <= getSize (operandType (IC_RIGHT (ic))) || ic->op == '+'))      // todo: More instructions that can write iy.
    {
      return (true);
    }
  ...
  if (!result_in_IY && !input_in_IY && (ic->op == '=' || ic->op == CAST && getSize (operandType (IC_RIGHT (ic))) >= 2 && (getSize (operandType (IC_RESULT (ic))) <= getSize (operandType (IC_RIGHT (ic))) || !IS_SPEC (operandType (IC_RIGHT (ic))) || SPEC_USIGN (operandType (IC_RIGHT (ic))))) && operand_is_pair (IC_RESULT (ic), a, i, G)) // DirSpace access won't use iy here.
    {
      return (true);
    }
...
DEinst_ok (const assignment & a, unsigned short int i, const G_t & G, const I_t & I)
{
  ...
  if ((ic->op == '=' || ic->op == CAST) && getSize (operandType (result)) >= 2 &&
      ...
      && (operand_on_stack (result, a, i, G) || operand_in_reg (result, REG_L, ia, i, G)
          || operand_in_reg (result, REG_H, ia, i, G)))
    {
      return (false);
    }
  ...
  if ((ic->op == '>' || ic->op == '<') && !SPEC_USIGN (getSpec (operandType (left)))
...
template < class I_t > float
weird_byte_order (const assignment & a, const I_t & I)
{
  float c = 0.0f;
...
template < class I_t > bool
local_assignment_insane (const assignment & a, const I_t & I, var_t lastvar)
{
...
assignment_hopeless (const assignment & a, unsigned short int i, const G_t & G, const I_t & I, const var_t lastvar)
{
  ...
  if (OPTRALLOC_HL &&
      (ia.registers[REG_L][1] >= 0 && ia.registers[REG_H][1] >= 0) &&
      (ia.registers[REG_L][0] >= 0 && ia.registers[REG_H][0] >= 0) && !HLinst_ok (a, i, G, I))
    {
      return (true);
    }

  // Can only check for IYinst_ok() in some cases.
  if (OPTRALLOC_IY && (ia.registers[REG_IYL][1] >= 0 || ia.registers[REG_IYH][1] >= 0) && !IYinst_ok (a, i, G, I))
    {
      return (true);
    }
      ...
      if (ai->s < ai_best->s)
        {
          ...
          for (vi = ai->local.begin (), vi_end = ai->local.end (); vi != vi_end; ++vi)
            if (ai->global[*vi] == REG_A || OPTRALLOC_HL && (ai->global[*vi] == REG_H || ai->global[*vi] == REG_L) || OPTRALLOC_IY
...
extra_ic_generated (iCode * ic)
{
  if (ic->op == '>' || ic->op == '<' || ic->op == LE_OP || ic->op == GE_OP || ic->op == EQ_OP || ic->op == NE_OP ||
      (ic->op == '^' || ic->op == '|' || ic->op == BITWISEAND) && (IS_OP_LITERAL (IC_LEFT (ic))
