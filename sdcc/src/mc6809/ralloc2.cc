...
static void extra_ic_generated(iCode *ic)
{
  ...
  if(ic->op == GET_VALUE_AT_ADDRESS)
    {
      iCode *inc;
      if (inc = hasIncmc6809 (IC_LEFT (ic), ic,  getSize (operandType (IC_RIGHT (ic)))))
         inc->generated = true;
    }
}
...
iCode *hc08_ralloc2_cc(ebbIndex *ebbi)
{
  ...
  mc6809_assignment_optimal = !tree_dec_ralloc(tree_decomposition, control_flow_graph, conflict_graph);
