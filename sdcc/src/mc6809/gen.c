...
forceload:
  ...
  switch (regidx)
    ...
    case H_IDX:
      ...
      else if (!(aop->op && isOperandVolatile (aop->op, FALSE)) && (loffset - 1 >= 0 || aop->type == AOP_LIT) && (aop->type == AOP_LIT || aop->type == AOP_IMMD || IS_HD6309 && aop->type == AOP_EXT))     /* TODO: Allow negative loffset - 1 */
        {
          bool pushedx = FALSE;
    ...
    case HX_IDX:
      if (aop->type == AOP_LIT)
        {
          loadRegFromConst (reg, byteOfVal (aop->aopu.aop_lit, loffset) | (byteOfVal (aop->aopu.aop_lit, loffset + 1) << 8));
          break;
        }
      if (aop->type == AOP_SOF)
        {
          int offset = (_G.stackOfs + _G.stackPushes + aop->aopu.aop_stk + aop->size - loffset - 1);
          if (IS_HD6309 && offset >= 0 && offset <= 0xff)
            {
      ...
      else if (aop->type == AOP_DIR || IS_HD6309 && aop->type == AOP_EXT)
        {
...
storeRegToAop (reg_info * reg, asmop * aop, int loffset)
{
  ...
  DD (emitcode ("", ";     storeRegToAop (%s, %s, %d), stacked=%d", reg->name, aopName (aop), loffset, aop->stacked));
  ...
  switch (regidx)
    ...
    case HX_IDX:
      ...
      if (aop->type == AOP_DIR || IS_HD6309 && aop->type == AOP_EXT)
        {
          emitcode ("sthx", "%s", aopAdrStr (aop, loffset, TRUE));
          regalloc_dry_run_cost += (aop->type == AOP_DIR ? 2 : 3);;
        }
      ...
      else if (IS_AOP_H (aop))
        {
          if (hc08_reg_a->isFree)
          ...
          else
            {
              ...
              emitcode ("tst", "1,s");
              regalloc_dry_run_cost += 4;
              if (IS_HD6309 && optimize.codeSpeed)
                {
                ...
                {
                  emitcode ("pulh", "");        // 1 Byte, 2 (hc08) cycles.
...
genCopy (operand * result, operand * source)
{
  ...
  if (operandsEqu (result, source) && !isOperandVolatile (result, FALSE) && !isOperandVolatile (source, FALSE))
    {
      return;
    }
  ...
  while (size)
    {
      if (size >= 2 && hc08_reg_h->isDead && hc08_reg_x->isDead &&
          (AOP_TYPE (source) == AOP_IMMD || AOP_TYPE (source) == AOP_LIT || IS_HD6309 && AOP_TYPE (source) == AOP_EXT) &&
          (AOP_TYPE (result) == AOP_DIR || IS_HD6309 && AOP_TYPE (result) == AOP_EXT))
        {
...
genCpl (iCode * ic)
{
  ...
  reg = (hc08_reg_a->isDead
         && !(AOP_TYPE (IC_RESULT (ic)) == AOP_REG
              && AOP (IC_RESULT (ic))->aopu.aop_reg[0] == hc08_reg_a) ? hc08_reg_a : hc08_reg_x);

  needpullreg = pushRegIfSurv (reg);
  while (size--)
    {
      bool needpullreg2 = (!size && AOP_TYPE (IC_RESULT (ic)) == AOP_REG && AOP (IC_RESULT (ic))->aopu.aop_reg[0] == reg || size
                           && AOP_TYPE (IC_RESULT (ic)) == AOP_REG && AOP (IC_RESULT (ic))->aopu.aop_reg[1] == reg);
      if (needpullreg2)
        {
          pushReg (reg, TRUE);
        }
...
genFunction (iCode * ic)
{
  ...
  emitcode (";", "-----------------------------------------");
  emitcode (";", mc6809_assignment_optimal ? "Register assignment is optimal." : "Register assignment might be sub-optimal.");
...
genPlusIncr (iCode * ic)
{
  ...
  if ((IS_AOP_HX (AOP (left)) || IS_AOP_HX (AOP (result)) ||
       ((AOP_TYPE (left) == AOP_DIR || IS_HD6309 && AOP_TYPE (left) == AOP_EXT)
        && (AOP_TYPE (result) == AOP_DIR || IS_HD6309 && AOP_TYPE (result) == AOP_EXT))) && (icount >= -128) && (icount <= 127)
      && (size == 2))
    {
  ...
  if (size == 1
      && (IS_AOP_X (AOP (result)) && (!IS_AOP_A (AOP (left)) || hc08_reg_h->isDead) || IS_AOP_X (AOP (left))
          && !IS_AOP_A (AOP (result)) && hc08_reg_x->isDead && hc08_reg_h->isDead))
    {
...
//  symbol *tlbl;
  ...
  if ((IS_AOP_HX (AOP (left)) || IS_AOP_HX (AOP (result)) ||
       ((AOP_TYPE (left) == AOP_DIR || IS_HD6309 && AOP_TYPE (left) == AOP_EXT)
        && (AOP_TYPE (result) == AOP_DIR || IS_HD6309 && AOP_TYPE (result) == AOP_EXT))) && (icount >= -127) && (icount <= 128)
      && (size == 2))
    {
      needpulx = pushRegIfSurv (hc08_reg_x);
      needpulh = pushRegIfSurv (hc08_reg_h);
  ...
  else if (size == 1 && IS_AOP_A (AOP (left)))
    {
      accopWithAop ("cmp", AOP (right), offset);
           ...
           && ((AOP_TYPE (right) == AOP_LIT)
               || ((AOP_TYPE (right) == AOP_DIR || IS_HD6309 && AOP_TYPE (right) == AOP_EXT) && (AOP_SIZE (right) == 2)))
           && (hc08_reg_h->isDead && hc08_reg_x->isDead || IS_AOP_HX (AOP (left))))
    {
      loadRegFromAop (hc08_reg_hx, AOP (left), 0);
...
genCmpEQorNE (iCode * ic, iCode * ifx)
{
  ...
  if ((size == 2)
      ...
      && ((AOP_TYPE (right) == AOP_LIT)
          || ((AOP_TYPE (right) == AOP_DIR || IS_HD6309 && AOP_TYPE (right) == AOP_EXT) && (AOP_SIZE (right) == 2)))
      && hc08_reg_h->isDead && hc08_reg_x->isDead)
    {
      loadRegFromAop (hc08_reg_hx, AOP (left), 0);
      emitcode ("cphx", "%s", aopAdrStr (AOP (right), 0, TRUE));
      regalloc_dry_run_cost += (AOP_TYPE (right) == AOP_DIR ? 2 : 3);
...
/*-----------------------------------------------------------------*/
/* hasIncmc6809 - operand is incremented before any other use        */
/*-----------------------------------------------------------------*/
iCode *
hasIncmc6809 (operand * op, const iCode * ic, int osize)
{
  ...
  if (ifx && AOP_TYPE (result) == AOP_CRY && AOP_TYPE (right) == AOP_LIT && AOP_TYPE (left) == AOP_DIR && bitpos >= 0)
    {
      ...
      if (IC_TRUE (ifx))
        {
          if (!regalloc_dry_run)
            emitcode ("brclr", "#%d,%s,%05d$", bitpos & 7, aopAdrStr (AOP (left), bitpos >> 3, FALSE),
        ...
        {
          if (!regalloc_dry_run)
            emitcode ("brset", "#%d,%s,%05d$", bitpos & 7, aopAdrStr (AOP (left), bitpos >> 3, FALSE),
                      labelKey2num ((tlbl->key)));
          regalloc_dry_run_cost += 3;
          emitBranch ("jmp", IC_FALSE (ifx));
          if (!regalloc_dry_run)
...
genSwap (iCode * ic)
{
  ...
  switch (AOP_SIZE (left))
    ...
    case 2:                    /* swap bytes in a word */
      if (IS_AOP_XA (AOP (left)) && IS_AOP_AX (AOP (result)) || IS_AOP_AX (AOP (left)) && IS_AOP_XA (AOP (result)))
        {
          break;
        }
...
genLeftShift (iCode * ic)
{
  ...
  if (sameRegs (AOP (right), AOP (result)) || regsInCommon (right, result) || IS_AOP_XA (AOP (result))
  ...
  if (hc08_reg_x->isDead && !IS_AOP_X (AOP (result)) && !IS_AOP_XA (AOP (result)) && !IS_AOP_AX (AOP (result))
...
genPointerGet (iCode * ic, iCode * pi, iCode * ifx)
{
  ...
  if (IS_BITVAR (retype))
    {
      /* hasIncmc6809() will be false for bitfields, so no need */
  ...
  else if (!pi && !ifx && size == 2 && IS_HD6309 && hc08_reg_x->isDead && hc08_reg_h->isDead && AOP_TYPE (result) == AOP_EXT)      /* Todo: Use this for bigger sizes, too */
    {
      loadRegIndexed (hc08_reg_hx, litOffset, rematOffset);
      storeRegToAop (hc08_reg_hx, AOP (result), 0);
    }
...
genAddrOf (iCode * ic)
{
  ...
  if (IS_AOP_HX (aopr) || aopr->type == AOP_DIR || (IS_HD6309 && aopr->type != AOP_REG))
    {
      needpullx = pushRegIfSurv (hc08_reg_x);
      needpullh = pushRegIfSurv (hc08_reg_h);
...
genAssignLit (operand * result, operand * right)
{
  ...
  if ((AOP_TYPE (result) != AOP_DIR) && IS_MC6809)
    {
      canUseHX = FALSE;
    }
      ...
      if (size == 1 && signExtend)
        {
          ...
          size--;
               ...
               (AOP_TYPE (right) == AOP_IMMD || IS_HD6309 && AOP_TYPE (right) == AOP_EXT) &&
               (AOP_TYPE (result) == AOP_DIR || IS_HD6309 && AOP_TYPE (result) == AOP_EXT))
        {
...
genReceive (iCode * ic)
{
  ...
  if (ic->argreg && IS_AOP_HX (AOP (IC_RESULT (ic))) && (offset + (ic->argreg - 1)) == 0)
  ...
  else if (ic->argreg)
    {
      while (size--)
        {
          if (AOP_TYPE (IC_RESULT (ic)) == AOP_REG && !(offset + (ic->argreg - 1))
              && AOP (IC_RESULT (ic))->aopu.aop_reg[0]->rIdx == X_IDX && size)
            {
              pushReg (hc08_reg_a, TRUE);
              delayed_x = TRUE;
            }
...
genhc08iCode (iCode * ic)
{
  ...
  switch (ic->op)
    ...
    case GET_VALUE_AT_ADDRESS:
      genPointerGet (ic, hasIncmc6809 (IC_LEFT (ic), ic, getSize (operandType (IC_RESULT (ic)))), ifxForOp (IC_RESULT (ic), ic));
      break;

    case '=':
      if (POINTER_SET (ic))
        {
          genPointerSet (ic, hasIncmc6809 (IC_RESULT (ic), ic, getSize (operandType (IC_RIGHT (ic)))));
        }
      else
        {
          genAssign (ic);
